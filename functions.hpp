...
void displayGray(   Image &img){
    ...
    imwrite("location on system\\pictures\\sample.jpg", ImgG);
    imshow("window name", ImgG);
    waitKey(0);
}
/*
splayGray(Image &img):
This function takes in an Image object and displays the grayscale representation.
A new Mat object (ImgG) is created with one channel (CV_8UC1) indicating it's a grayscale image.
img.getPixelVal(i, j) retrieves the grayscale value of the pixel at location (i,j) from the custom Image object.
The grayscale image is saved and displayed.*/
